...
fun main() = Task({
    ...
    file("AdventOfCode2023/in/day5/part1.in")
}){
    val maps = mutableListOf<Map>()
    var currentMap: Map? = null

    readValue()
    val seeds = readLongs().map { ChangingValue(it) }
    ...
    while (currentCategory != "location") {
        val useMap = maps.first { it.from == currentCategory }
        seeds.modify { useMap[it] }
        currentCategory = useMap.to
    }

    writeValue(seeds.minOf { it.value })
}